...
export const cetakBahanbakuMO = async () => {
  try {
    const res = await useClient.get("/cetak_laporan_bb_mo", {
      headers: {
        "Content-Type": "application/json",
        Authorization: `Bearer ${localStorage.getItem("token")}`,
      },
...
export const cetakBahanbakuOw = async () => {
  try {
    const res = await useClient.get("/cetak_laporan_bb_o", {
      headers: {
        "Content-Type": "application/json",
        Authorization: `Bearer ${localStorage.getItem("token")}`,
      },
